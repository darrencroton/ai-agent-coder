...
Do not request `scoped-implementation`, `code-simplifier`, `style-guide`, `commit`, `orchestrator`, `project-manager`, or any workflow requiring edits, external mutation, or supervisory authority from a read-only delegate. The launcher rejects these regardless: `commit`, `orchestrator`, `project-manager`, and `scoped-implementation` may never be embedded in any delegate prompt, in either access mode — the first three because they edit, mutate, or supervise on the Developer's behalf, and `scoped-implementation` because its own text is written for the Developer's first-person use and instructs "never let a Reviewer edit files," which directly contradicts a read-write delegate's own task. `code-simplifier` and `style-guide` are rejected for read-only specifically because they edit or create files — `style-guide`'s `audit` mode alone would be read-only-safe, but this mechanism has no per-mode granularity, so the whole skill is treated as write-only.

For a **read-write** delegate, `code-simplifier` is appropriate when the task is a cleanup pass and `style-guide` when the task is drafting or applying a style guide; the task's own `authorized_surface` and `non_goals` already state the bounded surface, so most read-write requests need no embedded skill at all. `commit` is never appropriate for any delegate, in either access mode — the Developer always owns the commit.
...
6. **Monitor** — use one managed `wait` with a timeout that fits the host's foreground-command limit. After a timeout, use `activity` only at the normal user-update cadence or to diagnose suspected trouble. Never loop short `wait`/`activity` calls: silence alone is not failure while the process remains healthy, and repeated polling adds no evidence. `wait` fails on `state=incomplete` too — a clean exit whose output never satisfied a declared `SECTION:` contract — not just on a nonzero returncode.
7. **Extract** — read the clean result and verify it against the requested output contract; a delegate is not a review until its output contract is satisfied.
