...
```text
codex exec <prompt> [-m <model>] [-c model_reasoning_effort="<effort>"] -c approval_policy="never" --sandbox read-only --skip-git-repo-check -C <repo>
```
...
```text
codex exec <prompt> [-m <model>] [-c model_reasoning_effort="<effort>"] -c approval_policy="never" --sandbox workspace-write --skip-git-repo-check -C <repo>
```
...
```text
codex exec resume <captured-id> <prompt> [-m <model>] [-c model_reasoning_effort="<effort>"] -c approval_policy="never" -c sandbox_mode="<mode>" --skip-git-repo-check
```

`codex exec resume` has a smaller flag set than `codex exec`: it rejects `--sandbox` and `-C` outright, and composing either fails the launch before the delegate starts. The sandbox is therefore expressed through its `sandbox_mode` config key on this path. Dropping `-C` costs nothing, because the launcher already starts every delegate process in the policy repository.
