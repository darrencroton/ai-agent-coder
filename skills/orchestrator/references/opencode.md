...
Write schema-v3 policy/request JSON as documented in [delegate-contract.md](delegate-contract.md), then use `delegate_jobs.py launch`. The launcher owns `opencode run`, prompt placement, the model flag, the effort flag, agent choice, auto approval, repository directory, and capture. A non-default effort composes `opencode run --variant <effort>`, which the CLI documents as "model variant (provider-specific reasoning effort, e.g., high, max, minimal)".
...
Discover exact model identifiers from the caller's OpenCode configuration or `opencode models`; do not guess. Pass explicit model choices through without capability ranking. OpenCode effort maps onto `--variant`; values are provider-specific and are passed through verbatim without capability ranking.
