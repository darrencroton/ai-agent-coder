...
- A deliberate, complete pass over naming/comment/documentation/logging consistency is the `style-guide` skill's `audit` mode, not yours — note an incidental style issue at `P3` if one is in front of you, but do not go looking for them; point the user at `style-guide` when they want that pass done properly.
- For broad, architectural, or maintainability-sensitive changes, run the `code-health` skill yourself if no report is already supplied: differentially against the review's base ref when one exists, or `--all` when reviewing completed/pre-existing code with no diff to compare against. Use it as structural evidence — investigate its measured candidates in context; do not repeat raw metric tails as review findings or treat its missing coverage as a clean result.
