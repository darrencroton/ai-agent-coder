...
- Batch corrections into one steer wherever the contract allows, rather than steering per finding: every correction is a budgeted resume turn, and on an elevated slice it also re-runs both mandatory reviews, since any tree change stales them. Batching corrections is not batching reviews — those stay sequential (see Risk).
