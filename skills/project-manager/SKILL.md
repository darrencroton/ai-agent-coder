...
4. **Run or supervise one slice** - launch a fresh tmux-backed harness session for one eligible slice. In model-supervised operation, keep the PM model in the loop to observe live pane/log/json/git evidence and choose safe operational actions.
...
7. **Repair, advance, or stop** - on a repairable gate with budget remaining, PM archives the stale result, writes a targeted repair prompt naming the exact violation, surfaces it into the **live** developer session (preserving the context the developer already built), and re-verifies the complete, unrelaxed gate when a fresh result lands — a repair can never lower the bar, only grant another chance to clear the identical one. A signature-keyed circuit breaker bounds the loop: first failure earns an in-session correction, the same signature failing again earns exactly one fresh-session retry with the frozen prompt plus targeted repair context and the cumulative residual-findings ledger, and a third consecutive failure is terminal regardless of remaining budget (`policy.max_repair_attempts`, default 3). PM advances to the next slice only when every gate passes, and stops with a precise reason for human approval, integrity breach, exhausted budget, harness failure, or incomplete evidence.
...
- `handoff`: records stop state and the next slice in Mode A. Mode B keeps continuation state in `run.json`, writes a PM-generated `slice-summary.md` for each terminal slice, and continuously refreshes `run-report.md` with gates, commits, blockers, next actions, and residual post-plan considerations.
