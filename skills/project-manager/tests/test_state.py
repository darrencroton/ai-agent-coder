...
class TestEventsAndReadback(PmTestCase):
    ...
    def test_read_events_skips_a_non_mapping_line(self) -> None:
        """A corrupted or hand-edited log can hold a bare array or scalar;
        every caller treats entries as mappings, so `.get()` several frames
        away is the wrong place to find that out."""
        plan_path = self.write_plan()
        _state, _token, run_dir = self.make_run(plan_path=plan_path)
        ...
        state_mod.append_event(run_dir, "observe", slice_id="Slice 1", note="ok")
