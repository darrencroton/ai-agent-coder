...
- `references/developer-prompt.md`: the rendered slice Developer prompt and repair-prompt contracts. Any `{placeholder}` change must match `render_developer_prompt`/`render_repair_prompt` in `runtime.py`; literal braces must be escaped. The prompt embeds only `orchestrator/references/pm-slice-contract.md`, not the full transitive skill bundle.
