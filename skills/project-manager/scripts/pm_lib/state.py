...
def read_events(run_dir: Path) -> list[dict[str, Any]]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        # A corrupted or hand-edited log can hold a bare array or scalar; every
        # caller treats entries as mappings, so skip anything else here rather
        # than let `.get()` raise several frames away.
