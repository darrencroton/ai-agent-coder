...
def add_harness_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--harness-model", help="model name/alias to compose through the PM harness profile, e.g. sonnet")
    parser.add_argument("--harness-effort", help="effort level to compose through the PM harness profile, e.g. medium")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="use PM's mechanical profile to compose the unattended harness command from run requirements",
    )
...
def add_unattended_default_arg(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        action="store_true",
        help="opt in to a known unattended-safe launch command for --harness codex/claude/copilot/opencode (disables per-action approval; PM's post-hoc gates become the safety boundary)",
    )


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Project Manager state and eligibility CLI")
    subparsers = parser.add_subparsers(dest="command", required=True)

    init = subparsers.add_parser("init", help="create a durable PM run")
    ...
    init.add_argument("--branch", help="intended branch to record for this PM run")
    init.add_argument(
        "--create-branch",
        action="store_true",
    ...
    check_parser = subparsers.add_parser(
        "check-plan",
        help="sanity-check a whole implementation plan before initializing a PM run",
    )
    ...
    for name, func, help_text in (
        ("status", status, "show current PM run state"),
        ("summarize", summarize, "summarize current PM run"),
    ):
        command = subparsers.add_parser(name, help=help_text)
        add_repo_run_args(command)
        command.set_defaults(func=func)

    profiles_parser = subparsers.add_parser("profiles", help="list PM harness launch mechanics")
    profiles_parser.set_defaults(func=list_profiles)

    preflight_parser = subparsers.add_parser("preflight", help="check the next PM slice launch before running it")
    ...
    stop_parser = subparsers.add_parser("stop", help="cancel the current PM run")
...
def main(argv: list[str] | None = None) -> int:
    if sys.version_info < (3, 13):
        print("pm: Python 3.13 or newer is required", file=sys.stderr)
        return 1
    parser = build_parser()
