...
> literal `none`; `{pm_surface_grants}` is a bulleted list of PM's recorded
> surface grants for this slice or the literal `none`; all wording stays in
> this file, including the authorization statement, so no reviewer-facing
> prose lives in `prompts.py`.
...
{changed_files}
Authorization: Project Manager checks the changed surface against the effective authorized surface itself — the frozen plan surface plus any grants listed below — as a non-waivable gate, so treat authorization as established unless your own reading of the pinned diff contradicts it — never caveat your conclusions on the absence of an independent audit report.
PM surface grants for this slice (the literal `none` if there are none). PM extended the effective authorized surface to these paths mid-run on recorded repository evidence; the frozen plan file itself is unchanged:
{pm_surface_grants}
