...
- **Wait events:** `note` carries stable `key=value` tokens; there is no separate structured field.
  - `observe` appends **one event per completed call** with a current slice, whether or not anything changed — a wait that returned nothing is exactly the call worth counting. Tokens: `pane_changed`, `liveness_changed`, `running`, `result_present`, `requested=<n>s|none`, `elapsed=<n>s`, and `wake=result|death|hard-stop|timeout|immediate` (why the call returned; `immediate` is an untimed observe that found no signal, and is not a wait). `result` outranks `death` when both hold. `wake` is never `pane`: the wait loop deliberately ignores pane byte-changes, since TUI spinner churn is not a completion signal. Events written before this existed carry no `wake` token, and anything counting them must treat those runs as lower bounds rather than guess.
  - `review-start` is appended when a reviewer process launches; a terminal `review` event carrying `status=success|timeout|nonzero` follows on the outcomes the command itself reaches. Both carry `seq=<n>`, and the pair is `(slice, seq)` — sequences are claimed per slice directory, so every slice can hold a commission `1`.
  - **Both are best-effort**, and deliberately so: a telemetry write that raised would be able to fail an observation that already happened, or orphan a reviewer that is already running. A busy state lock or an unwritable log therefore costs the event, not the operation. An unmatched `review-start` means **no terminal event was recorded** — the reviewer may still be running, may have died, or may have exited cleanly with the append or the post-processing failing afterwards. It is never proof of liveness in either direction.
  - Both feed the run report's **Wait Discipline** section. It counts actions, not turns: `events.jsonl` has no turn identifier, and harness-level polling never reaches it at all.
