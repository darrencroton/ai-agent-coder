...
- **Acceptance is recorded only against a quiesced repository.** The floor evaluation and the review-freshness gate both necessarily run while the Developer is still live, so they describe a tree it could still be changing. `finalize --accept` therefore terminates the tracked process group and then takes the evidence *again* — re-running the full floor, re-reading HEAD, and (on an elevated slice) re-checking that both mandatory reports are still fresh — and records the acceptance only if all three still hold. **It is this post-quiesce report and HEAD that the assessment records**, because they are the only pair measured against a repository no tracked process can still mutate. Three ways a Developer can lose an acceptance in that window, all refused: it committed (HEAD moved); it edited without committing (HEAD unchanged, but the clean-worktree fact trips) or emitted a hard-stop marker (fact 8 trips); or it altered a recorded review report — which moves neither HEAD nor the worktree and which no floor fact reads, so only the freshness re-check catches it. That last one is a real path rather than a hypothetical: review originals live under the run directory inside the worktree git dir, and a codex Developer is handed that git dir as an extra writable root so it can commit. On any of them no assessment is written and the slice stays undecided — the operator re-runs `finalize` to see the current evidence and then accepts or steers afresh. Note the asymmetry with the other two refusals: floor-failed and reviews-stale both refuse *before* termination and so deliberately leave the Developer alive to be steered, whereas detecting this race requires the Developer to be dead first. Steering still works after it, because a steer resumes by session id and launches a fresh turn.
...
- **Superseded attempts** live in `attempt-<n>/` subdirectories of the slice's `.pm/` artifact dir (the rotated `result.json`, `session-output.txt`, and `session-output.txt.exit`) and in the event log — never as state rows.
- **How a turn ended:** the turn's own shell records the harness's exit status to `session-output.txt.exit`, because a detached, reparented Developer cannot be `waitpid()`ed by a later `pm.py` invocation. `observe` reports it once the process is gone, distinguishing a non-zero exit and a signal death from an absent status — which means the group was terminated by PM or the machine rather than the harness ending itself. The guarantee is that the *foreground* command ended: PM's own composed launches are a single argv, but a `--harness-command` override that backgrounds its real work would report a clean exit while still running, which already defeats liveness tracking. It is diagnostic evidence for PM's judgement, not a floor fact, and it never changes attempt accounting — a turn that dies having produced nothing still costs an attempt, and a harness flaky enough to make that hurt is an argument for a higher `--max-attempts`, not for the toolkit guessing intent.
