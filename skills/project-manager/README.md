...
| `send --text T --reason R` | one-line nudge into the live session (refused while an interactive dialog marker is on the visible pane, so a blind Enter cannot answer a real dialog; the refusal names the matched literal and the way forward). Costs nothing |
