...
- Per slice: `prompt.md` (the rendered authorization), `session-output.txt` (the Developer's captured stdout/stderr), `session-output.txt.exit` (how the turn ended — see below), `status-before/after.txt`, `diff.patch`, `validation.md`, `result.json`, `observe-cursor.txt` (transient: the outfile byte count `observe` compares against), `attempt-<n>/` for superseded launches, `assessment.md` + `review-*.md` mirrors. Per run, not per slice: `developer.pid` — the sidecar carrying the tracked PID/PGID and start-time identity that lets `stop --scavenge` find and terminate a Developer even with run state deleted.

`session-output.txt.exit` records how a turn ended, written by the turn's own shell. The Developer is detached and reparented, so nothing can `waitpid()` it afterwards; without this, a session that died could only ever be reported as "not running". `observe` reads it and prints one of: the harness's exit code, `killed by signal N`, or — when no status was recorded — that PM or the machine terminated the group rather than the harness ending itself. It rotates into `attempt-<n>/` with the output it belongs to.

A harness that dies without producing a `result.json` still costs an attempt. If a particular harness proves unreliable enough that the budget is being spent on its flakiness rather than on the Developer's work, raise `--max-attempts` at `init` rather than expecting the toolkit to distinguish the two.
...
`scripts/verify_harness_argv.py` is a maintainer check with no CI role: it composes every harness's developer-launch, reviewer-launch, and resume argv through `profiles`, with the optional model and effort flags in separate variants, and checks that each composed flag appears in the installed CLI's own `--help` for the exact subcommand that receives it. For codex, opencode, and qwen — the three that only reveal a session id after launch — it also probes the session store `slice_ops` correlates against, running opencode's real correlation projections with `LIMIT 0` so a dropped *column* fails here rather than at `finalize --steer`, mid-run. Its precise guarantee is therefore: **flag presence and store structure — not flag values, flag combinations, `-c` config-key semantics, or how a value inside a stored record is encoded** (that last is the gap a real correlated session would have to close). Nothing unexercised counts as verified: a harness whose executable is absent, a store with no records, and qwen's per-directory store when the harness has not run in this repository are all reported as not checked rather than as passing or broken. The unit tests assert composed argv as strings, which cannot notice that a flag does not *exist* — that gap once let a codex resume shape frozen from documentation survive five slices before anyone ran it (`codex exec resume` rejects `--sandbox`, `-C`, and `--add-dir`). Run it after changing any command shape, and whenever a harness CLI is upgraded. It is a script rather than a test so the suite's no-skip guarantee and CI's hermeticity both survive.
