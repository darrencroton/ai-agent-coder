...
If the PM seat runs in Claude Code, consider installing the optional poll guard (`skills/project-manager/hooks/pm-poll-guard.py`). It blocks two ways a PM burns turns learning what the harness would have told it for free: a re-read of a background task's output returning bytes identical to the previous read, and a backgrounded command that sleeps and then inspects a PM artifact. Both are redundant, because Claude Code already re-invokes the agent when a background command exits, and expensive, because each one resends the whole conversation — about a quarter of the turns on one measured 1391-turn session. It fails open everywhere, and a foreground wait is always allowed. Setup: [`skills/project-manager/README.md`](skills/project-manager/README.md) → "Optional: the poll guard".
