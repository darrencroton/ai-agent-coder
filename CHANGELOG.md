...
- **Breaking — the Mode B Developer is headless; tmux is retired.** PM no longer runs the Developer in a persistent interactive tmux TUI. `start-slice` launches a detached background process with the launch pointer passed as the harness's own headless prompt argument (`-p`, `exec <prompt>`, `run <prompt>`, or `--prompt`, per harness), and the captured stdout/stderr (`session-output.txt`) is the universal, harness-agnostic progress signal for every tool. Readiness-banner detection, keystroke injection, and pane screen-scraping are deleted, along with the `pane-live.txt`/`pane.txt` captures and the free `send` nudge and its CLI subcommand — a headless turn runs to completion and exits, so every correction is now a budgeted `finalize --steer` resume turn, which first quiesces the prior turn — terminating and reaping its identity-checked process group if it is still live — so a resume can never race a still-acting turn. Liveness, termination, and `stop --scavenge` work from a tracked PID/PGID plus a start-time identity (guarding PID reuse) recorded in a `developer.pid` sidecar; `current_slice.tmux_session` is replaced by `session`, `session_id`, `pid`, `pgid`, and `outfile`. Steering fails closed when no launch-bound session id could be captured — PM never resumes "the last session." `tmux` is no longer a prerequisite, and no test self-skips for its absence.
- **OpenCode reasoning effort is first-class in PM.** `--effort` now composes `opencode run --variant <effort>` — the CLI's own "model variant (provider-specific reasoning effort)" control — for both the Developer and Reviewer seats, so Codex, Claude, Copilot, and OpenCode all honour an effort override through their native mechanism. Qwen Code exposes no reasoning flag at all, so `--effort` continues to fail closed there rather than being silently ignored.
