...
- A slice is **elevated** when the plan flags it (approval-gated, risky surfaces, independent review required), when PM escalates it on evidence (unexpected surface breadth, sensitive files, surprising diffs), or when PM grants it an additional path — a grant always ratchets to elevated. Elevated slices get independent review, deeper validation, and — where the plan says so — a human decision. PM may raise a slice's risk level on the record; it may never lower a plan-declared one.
