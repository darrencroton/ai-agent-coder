...
- No deletion of `test_state.py`'s `tmux_session` negative assertions or `run-state.md:66`/`:73` (renumbered from `:72`; see item A).
...
## Amendment 2026-07-26 (second) — Slice 7 absorbs finding F1 (owner-approved)

Slice 7 as first frozen above authorized three files and addressed only the accept-path TOCTOU window. During Slice 6 closeout a **P1 functional defect (F1)** was found that sits outside every slice's surface and would otherwise ship with the completed plan: `profiles.compose_resume_command("codex", …)` composes three flags that `codex exec resume` **rejects at argument parsing**, so `finalize --steer` against a codex Developer exits non-zero, does no work, and still spends an attempt. Because this plan deleted the free `send` nudge, a resume turn is the *only* correction mechanism — so **a codex Developer cannot be steered at all**. The owner decided on 2026-07-26 to fold F1 into Slice 7 rather than defer it, and to **preserve the linked-worktree commit capability via codex's `-c` config equivalent** rather than document it away.

This widens Slice 7's authorized surface to `profiles.py` and `test_profiles.py` and makes Slice 7 an in-session amended contract, which is why **the drift audit for Slice 7 runs at `medium`, not `low`** (authorized-surface reasoning stops being mechanical once the contract is amended mid-session). The code review stays at `high`.

### Why the `-c` equivalent, and what was actually measured

Against the installed `codex-cli 0.145.0`, `codex exec resume --help` lists `-c, --config <key=value>`. Both required overrides were verified — and, critically, verified *semantically*, not merely as argv the parser tolerates:

- `--strict-config` was first shown to genuinely validate `-c` overrides: a deliberately bogus key produced `Error loading config.toml: unknown configuration field 'totally_bogus_key_xyz' in -c/--config override`. So a silently-ignored-unknown-key false positive is ruled out.
- `-c sandbox_mode="workspace-write"` changed the startup banner from `sandbox: read-only` to `sandbox: workspace-write [workdir, /tmp, $TMPDIR]`.
- `-c sandbox_mode="workspace-write" -c 'sandbox_workspace_write.writable_roots=["/tmp/pm-probe-root"]'` produced `sandbox: workspace-write [workdir, /tmp, $TMPDIR, /tmp/pm-probe-root]` — the extra writable root demonstrably in effect. This is the `--add-dir` capability preserved, not approximated.

`-C <repo>` is dropped with nothing to replace it: `sessions.py:231` already passes `cwd=str(repo)` to `Popen`, so the resume turn's working directory — and therefore codex's `workdir` writable root — is the repo either way. `--skip-git-repo-check` is the one flag of the four that `codex exec resume` does accept, and it stays.

**`--strict-config` is deliberately NOT added to the composed command.** It would turn a future codex key rename into a loud failure instead of a silent capability loss, which is tempting given that silence is exactly how F1 survived five slices. But it also errors on *any* unrecognized field in the operator's own `~/.codex/config.toml`, so adding it would let an unrelated stale user config break every PM codex resume. The probe used it; the product does not.

**The launch path is unchanged.** `codex exec` (launch) does accept `-s/--sandbox`, `-C/--cd`, and `--add-dir`; only the `resume` subcommand is stricter. F1 is a resume-only defect and the fix is resume-only.

### Root cause, and what is deliberately NOT fixed here

No test ever feeds a composed command to a real CLI — `test_profiles.py` asserts argv *strings*, so a shape frozen from documentation survived five slices of review. An opt-in, not-in-CI smoke test asserting argv **acceptance** for all five harnesses × launch/resume is the real remedy. It is **out of scope for Slice 7** (new test infrastructure, network/CLI-dependent, and it would need all five CLIs installed) and is recorded as follow-up work for a future plan. Findings **F2** (the orchestrator's identical codex-continuation defect), **F3** (CHANGELOG `[Unreleased]` self-contradiction on tmux), and **F4** (the orchestrator's stale OpenCode effort claims) remain **out of scope and open** — F2 and F4 belong to `skills/orchestrator/`, a deliberately separate codebase, and F3's file is not in this slice's surface.

## Amendment 2026-07-26 (third) — Slice 7 closes the register outright (owner-approved)

Slice 7 is the plan's final slice, so "carry forward" is not available: an item left open here has no future slice to land in. The owner therefore directed that **every remaining open item be decided and fixed now**, rather than recorded for an uncertain future plan — "this is our opportunity to fix all issues to ensure correct, robust, vision aligned code and workflow". This amendment records that decision and the surface it authorizes. Nothing below is closed unfixed.

Items **G–I** are PM-side robustness gaps surfaced by the round-1 code review and the Developer's own fresh-eyes pass; **J** is the CHANGELOG defect (HANDOFF finding F3); **K** and **L** are the two `skills/orchestrator/` defects (HANDOFF findings F2 and F4). K and L are a deliberate cross-codebase widening: the orchestrator shares no code with PM, but F2 is a fully-broken path this very session had to work around, and leaving it would hand the next person a defect this plan already understood.

- **Item G — `finalize_stop` evidence pairing.** `finalize_stop` collects the floor report *before* termination but reads HEAD *after* it, so a STOPPED assessment can pair a pre-quiesce report with a post-quiesce HEAD — an internally inconsistent record. Fix: re-collect evidence after termination and render the assessment from that consistent pair. **`finalize_stop` must NOT start refusing**: a stop record exists to say what happened, floor passing or not. Only the evidence's internal consistency changes; the pre-termination floor event stays in the log as the record of what the Developer left behind.
- **Item H — risk-ratchet persistence on every fallible post-ratchet step.** `finalize_accept` and `finalize_stop` both apply the risk ratchet and durably append its `risk-raise` event *before* fallible evidence collection, but save state only later. An exception in between leaves the event log claiming an elevated slice while authenticated state still says `standard`, and a later acceptance would skip the very elevated review requirement the event announced. Round 1 guarded only the newly-added second collection in `finalize_accept`; the owner's close-everything direction supersedes the "pre-existing, leave alone" scope note recorded above.
  - *Corrected during round-2 review.* The first attempt — an `except PmError` guard wrapped around evidence collection — was **wrong, and green tests hid it**: `_collect_finalize_evidence` writes `status-after.txt`, so a Developer that deletes the artifact dir raises `FileNotFoundError`, not `PmError`, and `_terminate_current` raises from outside the wrapped region entirely. Guarding enumerated failures only ever covers the failures someone thought of. The fix is instead to **persist the ratchet immediately after its `risk-raise` event**, while the ratchet's risk fields are the only mutation on `state` — so no half-written decision can be persisted, and *every* later failure mode is covered rather than a chosen few. Applied uniformly to all three decision paths (`finalize_accept`, `finalize_stop`, `finalize_steer`). `finalize_steer`'s pre-existing `except PmError` guard is deliberately **kept**: it is still load-bearing for a different field, since the gates it wraps may have correlated and stored `session_id`, which a refusal should not discard.
- **Item I — the root cause behind F1: no composed argv was ever executed.** `test_profiles.py` asserts argv *strings*, so a shape frozen from documentation survived five slices of review. Fix: add `skills/project-manager/scripts/verify_harness_argv.py`, a standalone operator script that composes every harness × mode × launch/resume argv through the real `profiles` module and checks each flag against the installed CLI's own `--help`. **Deliberately NOT a pytest test**: the suite's "nothing skipped" invariant is a real guarantee this plan defends (Slice 4 removed the last `skipUnless`), and a CLI-dependent test would either skip or make CI non-hermetic. A script keeps CI hermetic and the check honest. This is not machinery for a hypothetical — F1 is the demonstrated failure it exists to prevent, so it is consistent with VISION principle 9 rather than an exception to it.
  - *Widened during round-2 review*, which found the first version gave partial assurance while reporting total success — the worst outcome for a verification tool. Three corrections: equals-style flags (`--resume=<id>`, copilot's resume form) are **normalized** rather than skipped, so that flag is genuinely checked; the optional **model and effort** flags are now composed too (`--effort`, `--variant`, `-c model_reasoning_effort`, `-m`/`--model`), since a per-harness effort flag frozen from a docs table is exactly the F1 failure mode and omitting it left the biggest hole; and the summary line now reports how many harnesses were actually checked and names any skipped for not being installed, so a skipped harness can never read as a verified one. A harness that legitimately fails closed on effort (qwen) simply contributes no effort variant.
  - *Corrected again during round-3 review*, which found that widening had introduced its own false-assurance hole: composing model and effort **together** meant qwen's expected effort refusal discarded the whole variant, taking its perfectly valid `--model` with it, while qwen still counted as checked. Model and effort are now composed as **separate** variants, and only an *effort* refusal is swallowed — any other `PmError` propagates rather than silently reducing coverage. Verified by mutation: pointing qwen's `headless_model_flag` at a bogus flag now reports `MISSING` and exits 1, where previously it passed. The README and CHANGELOG claims were tightened to state the script's precise guarantee — flag *presence* in the receiving subcommand's help, not flag values, combinations, or `-c` config-key semantics.
  - *Third correction, surfaced by running it rather than by review*: `copilot --help` began hanging on the development machine mid-session, and the script reported **"a frozen command shape would be REJECTED"** — a false alarm, because it conflated *"the CLI never answered"* with *"the CLI rejects this flag"*. A verification tool that cries wolf stops being read, which would quietly undo the whole point of item I. The script now distinguishes three outcomes: **rejected** (a real missing flag, exit 1), **inconclusive** (a CLI could not be queried — timeout or error — exit 2, naming which harnesses were and were not checked), and **verified** (exit 0, naming any harness skipped as not installed). The help timeout was also raised to 180s and the help cache keyed by the help *command* rather than by kind, so a harness without subcommands is not queried twice.
- **Item J — CHANGELOG `[Unreleased]` self-contradiction (HANDOFF F3).** `CHANGELOG.md:12` still says PM documentation "explicitly forbids acknowledging them through tmux" while `:13` announces tmux's retirement. Both sit in the same **unreleased** block, so this is not preserved release history — it would ship self-contradictory. Fix the stale clause; historical released entries stay untouched.
- **Item K — the orchestrator has PM's F1 defect independently (HANDOFF F2).** `delegate_contract.compose_delegate_command`'s codex branch appends `--sandbox <mode> --skip-git-repo-check -C <repo>` on **both** the launch and the `codex exec resume` continuation path, and `codex exec resume` rejects `--sandbox` and `-C` at argument parsing. So `-rN` continuation for a codex delegate cannot launch — demonstrated live during Slice 6 (`01-codex-slice6-drift-audit-r2` exited 2 with `error: unexpected argument '--sandbox' found`) and worked around again in Slice 7 by running every review round as a fresh session. Fix: on the continuation path only, express the sandbox as `-c sandbox_mode="<mode>"` and drop `-C`; `delegate_jobs.py:985` already passes `cwd=Path(contract["repo_path"])` to the child, so `-C` has nothing to replace. The launch path is unchanged — `codex exec` accepts all four flags.
- **Item M — line references this slice's own edit invalidated.** Slice 7 inserts a bullet into `references/run-state.md`, which shifts the second of Slice 6's two allowlisted contrastive tmux sentences from `:72` to `:73`. Slice 6's text referenced `:66`/`:72` in two places (item A and its non-goals) and would now misdirect a reader by one line. Both are corrected in place, with the reason recorded, so the correction is tracked rather than a silent rewrite of a completed contract. Found by the Developer's fresh-eyes pass, not by a delegate.
- **Item L(a), found during round-2 review.** `references/opencode.md`'s two displayed command shapes (lines 14 and 24) omitted the optional `--variant <effort>` even after the prose was corrected, leaving the reference internally inconsistent. Both shapes now show `[--variant <effort>]`.
- **Item L — the orchestrator's OpenCode effort claims are stale (HANDOFF F4).** `delegate_contract.py`'s opencode branch still fails closed on a non-default effort with "the tested OpenCode run command has no effort/variant flag", and `references/opencode.md:9,37` repeat the claim, and `tests/test_delegate_contract.py:486` pins it. Slice 6 established from the installed CLI that `opencode run --variant` **is** its reasoning-effort control. Fix: map effort onto `--variant` exactly as PM's item C did, correct both reference sentences, and replace the fail-closed test with one asserting the mapping. **Qwen's fail-closed effort handling in the orchestrator is unchanged** — its option list still contains nothing reasoning-related, exactly as in PM.

### Additional Authorized Surface (this amendment)
- `skills/project-manager/scripts/pm_lib/slice_ops.py` — items G and H: `finalize_stop`, and the ratchet guards in `finalize_accept`/`finalize_stop`.
- `skills/project-manager/scripts/verify_harness_argv.py` *(new)* and `skills/project-manager/README.md` — item I (the script and one line documenting it).
- `skills/project-manager/tests/test_finalize.py` — item G/H regression tests.
- `CHANGELOG.md` — item J, plus entries for this slice.
- `skills/orchestrator/scripts/delegate_contract.py`, `skills/orchestrator/references/opencode.md`, `skills/orchestrator/tests/test_delegate_contract.py` — items K and L.

### Additional Non-Goals (this amendment)
- **`finalize_stop` must not gain a refusal path.** It records; it does not decide.
- No change to the orchestrator's **launch** command shapes, to its policy/request schema, to its access-mode semantics, or to any harness other than codex (item K) and opencode (item L).
- No change to qwen's fail-closed effort handling in either codebase.
- No new CI job, and no test that skips — item I is a script precisely so the suite's no-skip invariant survives.
- No rewrite of released `CHANGELOG.md` history; only the `[Unreleased]` block's stale clause changes.

## Slice 7: Close the accept-path floor TOCTOU window (and fix F1, the codex resume shape)
...
- The post-quiesce report and HEAD become the authoritative ones recorded in the assessment: they are the only pair evaluated against a repository no *tracked* process can still mutate. (The guarantee is precisely that the tracked Developer process group is dead — not that nothing on the machine could write to the repo.)
- *Addendum, found during round-1 code review:* the re-check must also cover the **mandatory reviews on an elevated slice**, not only the floor and HEAD. `_is_review_fresh` re-hashes each report from disk, and review originals live under the run directory inside the worktree git dir — which a codex Developer is handed as an extra writable root so it can commit. A Developer that rewrites a required report on its way out moves neither HEAD nor the worktree, and no floor fact reads a review artifact, so the acceptance would otherwise proceed and the ACCEPTED assessment would render its own mandatory review as `[SUPERSEDED - stale]`. The freshness re-check is folded into the same `raced` refusal. Recorded here so the widened check is authorized rather than drift; it is a strict tightening of the accept gate and changes no passing acceptance.
- *Addendum, same review:* the new post-quiesce collection is fallible and sits after the risk ratchet has been applied and its event durably appended, so it carries the same `except PmError: save-if-ratcheted; raise` guard `finalize_steer` already uses. **Scope note:** the *first*, pre-existing `_collect_finalize_evidence` call has the identical exposure and is deliberately left alone — it predates this slice and fixing it would be a behaviour change to a path Slice 7 does not touch. Recorded as a decided, closed item, not an oversight.
...
- **Item F1 (added by Amendment 2026-07-26 (second)).** In `profiles.compose_resume_command`, replace the codex resume argv's three rejected flags with codex's `-c` config equivalents, keeping the accepted `--skip-git-repo-check`. `--sandbox workspace-write` becomes `-c sandbox_mode="workspace-write"`; the optional `--add-dir <git-dir>` becomes `-c sandbox_workspace_write.writable_roots=["<git-dir>"]`, emitted only when `git_access_dir` is supplied, exactly as the `--add-dir` form was; `-C <repo>` is dropped with no replacement because `Popen(cwd=repo)` already sets it. The `-c` value follows the module's existing single-argv-element idiom (`_append_headless_effort` already emits `-c model_reasoning_effort="E"` this way); the writable-roots value is a TOML array of strings generated by `json.dumps([str(git_access_dir)], ensure_ascii=False)` so paths are escaped rather than concatenated. **`ensure_ascii=False` is load-bearing, not cosmetic** (found during round-1 code review): the default escapes a non-BMP character — an emoji in a worktree path — into a `\uXXXX` surrogate pair, and TOML rejects surrogates as not being Unicode scalar values, so such a resume would die in codex's config parser. Regression-tested with an emoji path parsed through `tomllib`.

### Frozen Command Shapes (amending Slice 2's table for the codex *resume* row only)
- codex resume — `codex exec resume <session-id> <correction> -c sandbox_mode="workspace-write" [-c sandbox_workspace_write.writable_roots=["<git-dir>"]] --skip-git-repo-check`
- codex **launch** — **unchanged** from Slice 2: `codex exec <pointer> [-m M] [-c model_reasoning_effort="E"] --sandbox workspace-write --skip-git-repo-check -C <repo> [--add-dir <git-dir>]`. `codex exec` accepts all four; only `codex exec resume` is stricter.
- The other four harnesses' launch and resume shapes are **unchanged**, byte for byte.
...
- **F1:** `compose_resume_command("codex", …)` emits the frozen shape above and contains none of `--sandbox`, `-C`, `--add-dir`; with `git_access_dir=None` it emits no `writable_roots` override; `finalize --steer` against a codex Developer composes an argv the installed CLI accepts. The other four harnesses' resume argv and all five launch argv are unchanged.
...
- Files allowed to change:
  ...
  - `skills/project-manager/tests/test_finalize.py`
  - `skills/project-manager/references/run-state.md`
  - `skills/project-manager/scripts/pm_lib/profiles.py` *(added by Amendment 2026-07-26 (second), item F1)*
  - `skills/project-manager/tests/test_profiles.py` *(added by Amendment 2026-07-26 (second), item F1)*
  - `docs/implementation-plan-headless-developer.md` *(this amendment)*
- Functions/classes/components allowed to change: `finalize_accept` and the `AcceptOutcome` dataclass (a new refusal `kind`) in `slice_ops.py`; the accept-semantics paragraph in `run-state.md`; **the codex branch of `compose_resume_command` and its docstring in `profiles.py` only** — `compose_headless_command`, `HARNESS_PROFILES`, `_append_headless_effort`, and every non-codex resume branch are out of bounds.
...
### Risk Flags
- Risky surfaces touched: the accept decision path — the point at which PM records an irreversible assessment — and the frozen per-harness command table (codex's resume row).
- Approval needed before implementation: **yes — both this slice's original amendment and Amendment 2026-07-26 (second)**, granted 2026-07-26.
- Independent audit required: yes — two separate read-only delegates, **drift-audit at `medium`** (raised from `low` because F1 makes this an in-session amended contract) **then code-review at `high`**.

### Validation Plan
- Tests to add/update: a fake harness that writes `result.json`, commits, then keeps committing until killed, so HEAD provably differs between the two evaluations → acceptance refuses with the race message and writes no assessment; an ordinary accept still succeeds (proving the second floor evaluation is not spuriously failing — note `_collect_finalize_evidence` writes into the artifact dir on both runs, so this test is the guard against the second run's own writes flipping a fact); floor-failed and reviews-stale refusals still leave the Developer alive. **F1:** `test_profiles.py`'s frozen codex resume row updated to the shape above, plus assertions that the rejected flags are absent and that `git_access_dir=None` omits the writable-roots override.
- Added by the round-1 addenda: a raced-acceptance test in which the Developer **corrupts a mandatory review report** on its way out (HEAD unchanged, worktree clean, floor 8/8 — so only the freshness re-check can catch it), and a `test_profiles.py` regression pinning that an **emoji worktree path** survives as parseable TOML rather than a surrogate pair.
- **The race tests must be proven non-vacuous**: each must be shown to FAIL when the check it exercises is removed. A green suite around a vacuous race test is this slice's primary risk. Both mutation proofs were run: deleting the floor/HEAD re-check fails the commit and edit tests, and deleting the review-freshness re-check fails the review-corruption test — in each case the pre-fix code visibly prints `ACCEPTED` on evidence that had already gone stale.
- Added by Amendment (third): a `finalize --stop` test in which the Developer commits an **unauthorized** file on its way out — the pre-termination floor is a genuine 8/8 and the post-quiesce floor genuinely fails the surface fact, so the assessment must describe the tree its recorded HEAD actually names (item G); a ratchet-persistence test covering **both** decision paths, asserting the ratchet survives a raising evidence collection (item H); an orchestrator test pinning that the codex *continuation* omits `--sandbox`/`-C` while the launch keeps them (item K); and an orchestrator test asserting `--variant` now carries effort for opencode, replacing the one that pinned the stale fail-closed claim (item L).
- Commands to run: the full suite `uv run --python 3.13 --with pytest python -m pytest skills/project-manager/tests/`, unsandboxed, nothing skipped (baseline 313); **plus** `uv run --python 3.13 --with pytest python -m pytest skills/orchestrator/tests/` for items K and L; **plus** `python3 skills/project-manager/scripts/verify_harness_argv.py` exiting 0 (item I).
- Manual checks: read `finalize_accept` end to end and confirm the ordering comment still describes the code; the composed codex resume argv is accepted by the installed `codex exec resume` (argument parsing reached session lookup, not an `unexpected argument` error).

### Rollback Path
- Revert the slice commit; `finalize_accept` returns to a single pre-termination floor evaluation and the codex resume argv returns to its (invalid) Slice 2 shape. Slice 6 is unaffected.
